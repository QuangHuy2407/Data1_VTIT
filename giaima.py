...
CLICKHOUSE_HOST = "10.10.112.102"
CLICKHOUSE_PORT = int(os.getenv("CLICKHOUSE_PORT", 8123))
CLICKHOUSE_DB = os.getenv("CLICKHOUSE_DB", "Customer")
...
def main():
    ...
    # --- BƯỚC 2: KIỂM TRA MỐC DỮ LIỆU ĐÃ GIẢI MÃ LỚN NHẤT (CƠ CHẾ DELTA LOAD) ---
    ...
    # --- BƯỚC 3: XÂY DỰNG TRUY VẤN LỌC DỮ LIỆU TỪ BẢNG GỐC ---
    select_fields = """
    ...
    for row in result.result_rows:
        ...
        if decrypted_age_str or decrypted_postal_str or decrypted_reward_str:
            success_count += 1
    ...
    print(f"--- Giải mã thành công xuất sắc: {success_count:,} / {total_rows:,} dòng ---")
